...
class SmallTalkPage extends StatefulWidget {
  @override
  State<StatefulWidget> createState() =>SmallTalkState();
}

class SmallTalkState extends State<SmallTalkPage>{
  int view = 5;
  void initState() {
    super.initState();
  }
          ...
          ListTile(
            ...
            trailing: article.uid == cu.currentUser.getUid()?IconButton(icon: Icon(Icons.close),
              onPressed: (){
                ...
                StorageReference storageRef = FirebaseStorage.instance.ref().child('/smallTalk/${article.id}');
                for(int i = 0;i < count; i++){
                  storageRef.child('$i').delete();
                }
                Firestore.instance.collection('club').document('슬기짜기').collection('smallTalk').document(article.id).delete();
              },
            ):SizedBox(),
          ),
          Container(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      floatingActionButton: FloatingActionButton(
        ...
        onPressed: (){
          Navigator.push(context,
            MaterialPageRoute(builder: (BuildContext context) => AddAnnouncementPage(from:2)),
          );
        },
      ),
      body: Stack(
            ...
            body: Column(
              children: <Widget>[
                ...
                StreamBuilder<QuerySnapshot>(
                  stream: Firestore.instance.collection('club').document('슬기짜기').collection('smallTalk').orderBy("id", descending: true).limit(view).snapshots(),
