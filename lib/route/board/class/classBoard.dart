...
class ClassBoardPage extends StatefulWidget {
  @override
  State<StatefulWidget> createState() =>ClassBoardState();
}

class ClassBoardState extends State<ClassBoardPage>{
  int view = 5;
  void initState() {
    super.initState();
  }
          ...
          ListTile(
            ...
            trailing: article.uid == cu.currentUser.getUid()?IconButton(icon: Icon(Icons.close),
              onPressed: (){
                ...
                StorageReference storageRef = FirebaseStorage.instance.ref().child('/classBoard/${article.id}');
                for(int i = 0;i < count; i++){
                  storageRef.child('$i').delete();
                }
                Firestore.instance.collection('club').document('슬기짜기').collection('classBoard').document(article.id).delete();
              },
            ):SizedBox(),
          ),
          Container(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      floatingActionButton: FloatingActionButton(
        ...
        onPressed: (){
          Navigator.push(context,
            MaterialPageRoute(builder: (BuildContext context) => AddAnnouncementPage(from:4)),
          );
        },
      ),
      body: Stack(
            ...
            body: Column(
              children: <Widget>[
                ...
                StreamBuilder<QuerySnapshot>(
                  stream: Firestore.instance.collection('club').document('슬기짜기').collection('classBoard').orderBy("id", descending: true).limit(view).snapshots(),
